...
Widget _buildCoinSelector() {
  return Observer(
    builder: (_) {
      ...
      // Verificar se o valor atual existe na lista
      final validValue = controller.availableCoins.contains(controller.currentCoinId)
      ...
      return DropdownButtonFormField<String>(
        ...
        items: controller.availableCoins.map((coinId) {
  ...
  Widget _buildPeriodSelector() {
    return Observer(
      builder: (_) => SegmentedButton<String>(
        segments: const [
        ...
        onSelectionChanged: (periods) => controller.changePeriod(periods.first),
      ),
    );
  }
      ...
      bottomTitles: AxisTitles(
        sideTitles: SideTitles(
          ...
          getTitlesWidget: (value, _) {
            ...
            final date = controller.selectedPeriod == '1D'
                ? DateTime.now().subtract(Duration(hours: 24 - index))
                : DateTime.now().subtract(Duration(days: historyLength - index - 1));
